...
## 14. Ближайшая точка старта

Текущий репозиторий находится раньше Этапа 0:

- health service пока возвращает ошибку-заглушку;
- users transport не завершён и не зарегистрирован;
- PostgreSQL и S3 отсутствуют;
- есть незавершённые изменения HTTP response/middleware;
- текущая User-модель не соответствует разделению Account и Person;
