...
- `include_partners` — при `true` добавляет активные союзы участников родительского фрагмента и всех активных участников этих союзов одним переходом; по умолчанию `false`.
...
```json
{
  ...
  "union_members": [
    {
      "union_id": "342f2a90-8656-4a23-9034-2f7964832764",
...
Персоны представлены краткими карточками с preferred-именем. Если партнёры не запрошены, `unions` и `union_members` — пустые массивы. Graph query выполняется в read-only repeatable-read транзакции. Сервер возвращает не более `500` уникальных персон с учётом партнёров; превышение лимита даёт `422`, после чего клиенту следует уменьшить глубину или отключить партнёров.
