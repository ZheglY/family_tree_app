...
ALTER TABLE export_jobs
    ADD CONSTRAINT export_jobs_format_check
        CHECK (format IN ('json_backup', 'zip_backup', 'pdf', 'png', 'svg', 'gedcom', 'gedzip')),
        ...
        CHECK (
            status <> 'completed' OR (
                ...
                (
                    ...
                    (format = 'gedcom' AND result_mime_type = 'text/vnd.familysearch.gedcom') OR
                    (format = 'gedzip' AND result_mime_type = 'application/vnd.familysearch.gedcom+zip')
                ) AND
