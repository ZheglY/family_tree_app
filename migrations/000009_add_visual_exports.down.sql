...
WHERE export.format IN ('pdf', 'png', 'svg')
  AND job.kind IN ('export.generate', 'export.delete')
  AND job.payload ->> 'export_id' = export.id::text;

DELETE FROM export_jobs WHERE format IN ('pdf', 'png', 'svg');
...
ALTER TABLE export_jobs
    ADD CONSTRAINT export_jobs_format_check
        CHECK (format IN ('json_backup', 'zip_backup')),
        ...
        CHECK (
            status <> 'completed' OR (
                ...
                (
                    ...
                    (format = 'zip_backup' AND result_mime_type = 'application/zip')
                ) AND
